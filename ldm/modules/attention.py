...
class CrossAttention(nn.Module):
    ...
    def forward(self,
        ...
        # Remove q_mix since we won't use it for Q fusion
        if injection_config is not None:
            attn_matrix_scale = injection_config['T']
            # Removed: q_mix = injection_config['gamma']

        # Modified Q processing - no fusion, just direct use
        if q_injected is None:
            # Normal Q computation when no injection
            q = self.to_q(x)
            q = rearrange(q, 'b n (h d) -> (b h) n d', h=h)
        else:
            # Direct use of injected Q without any mixing
            q_uncond = q_injected
            q = torch.cat([q_uncond]*b, dim=0)

        context = default(context, x)

        # K processing remains the same
