...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape, negative_conditioning=None,
        ...
        for i, step in enumerate(iterator):
            ...
            # Detailed debug info
            if i < 3 or i >= total_steps - 3:
                print(f"\n[Sampling] i={i}, step={step}, index={index}")
                
            if index > start_step:
                if i == 0:
                    print(f"Skipping steps where index >= {start_step}")
                continue
            ...
            if injected_features_i is not None and (i < 3 or i >= total_steps - 3):
                print(f"  Injecting features at index: {i}")
                if 'output_block_6_self_attn_q' in injected_features_i:
                    print(f"  Contains output_block_6 Q features")
    ...
    def encode_ddim(self, img, num_steps, conditioning=None, unconditional_conditioning=None,
        ...
        print(f"\n=== Starting DDIM Inversion (num_steps={num_steps}) ===")
        
        # First calculate callback_ddim_timesteps_list
        callback_ddim_timesteps_list = np.flip(make_ddim_timesteps("uniform", callback_ddim_timesteps, self.ddpm_num_timesteps))\
            if callback_ddim_timesteps is not None else np.flip(self.ddim_timesteps)
        ...
        if num_steps == 999:
            ...
            # Add debug prints
            print(f"Inversion timesteps: {list(time_steps)[:5]}...{list(time_steps)[-5:]}")
            print(f"Callback timesteps: {callback_ddim_timesteps_list[:5]}...{callback_ddim_timesteps_list[-5:]}")
        ...
        for i, t in enumerate(iterator):
            ...
            # Debug prints
            if i < 3 or i >= num_steps - 3:
                print(f"[Inversion] Step i={i}, timestep t={t}, t_next={steps[i+1] if i+1 < len(steps) else 'N/A'}")
